defmodule RemindersonWeb.TweetReminderLiveTest do
  ...
  @create_attrs %{
    ...
    remind_at: %{day: 26, hour: 20, minute: 32, month: 1, year: 2022},
    reminder_id: 42,
    tags: [],
    text: "some text"
  }
  @update_attrs %{
    ...
    remind_at: %{day: 27, hour: 20, minute: 32, month: 1, year: 2022},
    reminder_id: 43,
    tags: [],
  ...
  @invalid_attrs %{
    ...
    remind_at: %{day: 30, hour: 20, minute: 32, month: 2, year: 2022},
    reminder_id: nil,
    tags: [],
    text: nil
  }
  ...
  describe "Index" do
    ...
    test "updates tweet_reminder in listing", %{conn: conn, tweet_reminder: tweet_reminder} do
      ...
      assert index_live
             |> element("#tweet_reminder-#{tweet_reminder.id} a", "Edit")
             |> render_click() =~
               "Edit Tweet reminder"
    ...
    test "deletes tweet_reminder in listing", %{conn: conn, tweet_reminder: tweet_reminder} do
      ...
      assert index_live
             |> element("#tweet_reminder-#{tweet_reminder.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays tweet_reminder", %{conn: conn, tweet_reminder: tweet_reminder} do
      {:ok, _show_live, html} =
        live(conn, Routes.tweet_reminder_show_path(conn, :show, tweet_reminder))
